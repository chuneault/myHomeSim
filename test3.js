...
function compareFaces() {
    ...
    let params = {
        SourceImage: {
            Bytes: base64Encode(savePath+'lauralie1.jpg')
        },
        TargetImage: {
            Bytes: base64Encode(savePath+'lauralie2.jpg')
        },
...
//indexFace(savePath+'alexandra1.jpg', 'alexandraId');
//searchFacesByImage(savePath+'cahrlineelianelauralie.jpg');
...
function checkFacesFromCam(callback, maxCountGetPhoto = 3){
    ...
    function checkCamPic() {
        getCamPic((err, resp, data) => {
            fs.writeFileSync(savePath+'face/camPicOrig' + getCountPhoto + '.jpg', data,  'binary');
...
//searchFacesByImage(savePath+'elianemarilou.jpg');
//searchFacesByImage(savePath+'face/camPicOrig0-0.jpg');


/*
detectFaces(savePath+'elianemarilou.jpg', 'test',  function(err, similarity, face, index) {
	if (err) {
	   if (err.message != 'There are no faces in the image. Should be at least 1.')
...
var params = {
  OutputFormat: "mp3", 
  SampleRate: "8000", 
  Text: "Salut, je m'appelle Chantal. Je vais lire le texte que vous écrirez ici.", 
  TextType: "text", 
  VoiceId: "Chantal",
  LanguageCode: "fr-CA"
 };
 ...
 polly.synthesizeSpeech(params, function(err, data) {
   ...
   fs.writeFileSync(savePath+'audio/polly.mp3', data.AudioStream, 'binary');
