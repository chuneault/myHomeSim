...
class hsController {
  ...
  constructor(logger) {
    ...
    this.event.setMaxListeners(100);
      ...
      schedule.scheduleJob('15 * 0 * * *', function () {
          console.log('another day begin!');
          this.event.emit('newDay');
      });
  };
  ...
  addNode(newNode, ownerDevice) {
    ...
    newNode.__type = 'node';
    //newNode.toJSON = this.getNodeInfoJSON;
    ...
    if (ownerDevice) {
      ...
      newNode._deviceId = ownerDevice._id;
    }
  ...
  addSensor(newSensor, ownerNode) {
    ...
    newSensor._nodeId = ownerNode._id;
  ...
  runScript(script, sandboxVar) {
    ...
    if ((runScript.__type == 'script') && (runScript.script))
      runScript = runScript.script;
    else
    if ((runScript.__type == 'script') && (runScript.scriptFileName))
