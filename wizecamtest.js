...
//indexFace('/tmp/img/marilou1.jpg', 'marilouId');

//indexFace('/tmp/img/carl.jpg', 'carlId');

//searchFacesByImage('/tmp/img/cahrlineelianelauralie.jpg');

//searchFacesByImage('/tmp/img/carltest.png');
			...
			detectFaces(data, 'camPicOrig' + getCountPhoto, (err, similarity, face, index, last) => {
			  ...
			  if (last) {
				...
				if (getCountDetectFaces >= maxCountGetPhoto)
			      setTimeout(()=>{console.timeEnd('checkFacesFromCam');}, 5); 
			  }
			});		
			getCountPhoto+=1;
...
/*checkFacesFromCam((err, similarity, face) => {
  if (err) console.log(err);
  else
   console.log((Math.trunc(similarity) + '%').yellow, face.ExternalImageId.green);	  
});*/

searchFacesByImage('/mnt/faces/18-12-04_7.30.30-face.jpg');
