...
class hsController {
      ...
      if (nodeExpired.length > 0) {
        ...
        _.forEach(nodeExpired, function(node) {
          ...
          node.__ownerDevice.__controller.invokeAction('kik', 'sendMessage', [
              format('Last Heart Beat for {} was {}', node.name, moment(node.lastHeartBeat).fromNow()), 'carlturtle37'
          ]);
  ...
  checkJSONItemToRemove(obj, deleteItem = false){
    let self = this;
    let result = {};
    _.forEach(obj, function(val, key){
        ...
        /*else
          if ((!_.startsWith(key, '_')) && (typeof obj[key] == 'object'))
            self.checkJSONItemToRemove(obj[key]);*/
    });
    return result;
  };
  ...
  updateNode(node, updateProperties) {
    ...
    this.event.emit('updateNode', node, this.checkJSONItemToRemove(node));

    this.checkJSONItemToRemove(node, true);
    return node;
  }
  ...
  updateSensor(sensor, updateProperties) {
    ...
    this.checkJSONItemToRemove(sensor);
