                      ...
                      let updateNodeDB = function() {
                          ...
                          self.db.nodes.get(node._id).then(
                              function(doc) {
                                  ...
                                  let updateNode = serializeObj(node);
                                  //console.log('updateNode', updateNode);
                                  if (updateNode.vendor) {
                                      if (doc.vendor)
                                          _.extend(doc.vendor, updateNode.vendor);
                                      else
                                          doc.vendor = updateNode.vendor;
                                      delete (updateNode.vendor);
                                  }
                                  _.extend(doc, updateNode);
                                  doc._rev = lastRev;
                                  //console.log('updateNode BD', doc);
                                  return self.db.nodes.put(doc);
                              }).then(function(response) {
                                // handle response
                                console.log('updateNode BD', response);
                  ...
                  controller.on('updateSensor', function (sensor) {
                      ...
                      self.db.sensors.get(sensor._id, function(error, doc) {
                          if (error) {
                              ...
                              let lastRev = doc._rev;
                              let updateSensor = serializeObj(sensor);
                              if (updateSensor.vendor) {
                                  if (doc.vendor)
                                      _.extend(doc.vendor, updateSensor.vendor);
                                  else
                                      doc.vendor = updateSensor.vendor;
                                  delete (updateSensor.vendor);
                              }
                              _.extend(doc, updateSensor);
                              doc._rev = lastRev;
