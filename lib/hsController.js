...
class hsController {
    ...
    constructor(logger, config) {
        ...
        this.objects['controller'] = {class: this, turnOnOffSensor: self.turnOnOffSensor, turnOnSensor: self.turnOnSensor, turnOffSensor: self.turnOffSensor};
    ...
    turnOnOffSensor(id, fromSensor) {
        console.log('turnOnOffSensor', id);
        let sensor = this.sensors[id];
        if (sensor) {
            if (fromSensor.lastValue == true || fromSensor.lastValue == 'ON')
                sensor.turnOn();
            else
                sensor.turnOff();
        }
    ...
    turnOnSensor(id, fromSensor) {
        console.log('turnOnSensor', id);
        let sensor = this.sensors[id];
        if (sensor)
          sensor.turnOn();
    }

    turnOffSensor(id, fromSensor) {
        console.log('turnOffSensor', id);
        let sensor = this.sensors[id];
        if (sensor)
            sensor.turnOff();
    }
        ...
        let check = function (nodes) {
            ...
            if (nodeExpired.length > 0) {
                ...
                _.forEach(nodeExpired, function (node) {
                    ...
                    node.__ownerDevice.__controller.invokeAction('kik', 'sendMessage', [
                        format('Last Heart Beat for {} was {}', node.desc ? node.desc : node.name, moment(node.lastHeartBeat).fromNow()), 'carlturtle37'
                    ]);
                    console.log('Please Check Last Heart Beat'.bold.red, 'for', node.desc ? node.desc.bold.green : node.name.bold.green, 'since:', moment(node.lastHeartBeat).fromNow().bold.yellow);
    ...
    updateNode(node, updateProperties, updateLastUpdate = true) {
        ...
        if (updateLastUpdate)
            ...
            _.extend(node, updateProperties);

        let unsetFields = this.checkJSONItemToRemove(node);

        this.event.emit('updateNode', node, unsetFields);
    ...
    addOrUpdateNode(filter, nodeProperties, ownerDevice, callback, updateLastUpdate = true) {
        var self = this;
        self.findNode(filter, function (notFound, node) {
            if (notFound)
                node = self.addNode(nodeProperties, ownerDevice);
            else
                node = self.updateNode(node, nodeProperties, updateLastUpdate);
            if (callback)
                callback(null, node);
        });
    }
    ...
    addOrUpdateSensor(filter, sensorProperties, ownerNode, callback, updateLastUpdate = true) {
        var self = this;
        self.findSensor(filter, function (notFound, sensor) {
            if (notFound)
                sensor = self.addSensor(sensorProperties, ownerNode);
            else
                sensor = self.updateSensor(sensor, sensorProperties, updateLastUpdate);
            if (callback)
                callback(null, sensor);
        });
    }

    updateSensor(sensor, updateProperties, updateLastUpdate = true) {
        if (updateProperties.vendor) {
            if (sensor.vendor)
        ...
        if (updateLastUpdate)
            _.extend(sensor, updateProperties, {lastUpdate: _.now()});
        else
            _.extend(sensor, updateProperties);

        let unsetFields = this.checkJSONItemToRemove(sensor);

        this.event.emit('updateSensor', sensor, unsetFields);
    ...
    findSensor(filter, callback) {
        let found = _.find(this.sensors, filter);
        if (callback) {
            if (found)
                callback(null, found);
            else {
    ...
    deleteSensor(sensor, updateProperties) {
        if (sensor.__ownerNode)
            delete sensor.__ownerNode.__sensors[sensor._id];
        delete this.sensors[sensor._id];
        this.event.emit('deleteSensor', sensor);
        return true;
    }
    ...
    addSensorValue(sensor, value) {
        ...
        if (sensor.actionOnChange) {
            if (sensor.lastValue != sensor.previousValue) {
                let actionsargv = s(sensor.actionOnChange).parseCSV();
                this.log.info('actionOnChange', actionsargv[0], actionsargv[1], actionsargv.slice(2));
                this.invokeAction(actionsargv[0], actionsargv[1], actionsargv.slice(2), sensor);
            }
        }

        if (sensor.scriptOnChange) {
            this.runScript(sensor.scriptOnChange, {
                sensor: sensor,
                value: sensor.lastValue, previousValue: sensor.previousValue
            });
        }
    ...
    addFileAttachement(data, callback) {
        this.event.emit('newFileAttachement', data, callback);
    }

    getFileAttachement(fileId, callback) {
        this.event.emit('getFileAttachement', fileId, callback);
    };
    ...
    runScript(script, sandboxVar) {
        ...
        try {
            ...
            this.log.profileTime('run script');
            ...
            else if ((runScript.type == 'script') && (runScript.scriptFileName))
                runScript = fs.readFileSync(runScript.scriptFileName, 'utf8');

            const sandbox = _.extend({
                Buffer: Buffer,
                server: this,
                format: require('string-format'),
        ...
        this.log.profileEnd('run script');
    }

    invokeAction(name, method, arrayArgv, fromSensor) {
        try {
            let classObj;
            if (classObj = this.objects[name.toLowerCase()]) {
                this.log.profileTime('run action');
                this.log.info('Running action', name, method);
                let obj = classObj.class;
                if (fromSensor)
                    arrayArgv.push(fromSensor);
                obj[method].apply(obj, arrayArgv);
                this.log.profileEnd('run action');
            }
        }
        catch (error) {
            console.error(error);
